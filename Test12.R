...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      checkboxGroupInput("plans", "Select Student Loan Plans You Repay",
                         choices = list(
...
output$premia_selector <- renderUI({
  ...
  tagList(
    checkboxGroupInput("flexible_pay_premia", "Flexible Pay Premia Type",
                       choices = choices,
                       selected = if (grade %in% c("FY1", "FY2")) "none" else NULL),
    bsPopover("flexible_pay_premia",
              title = "Flexible Pay Premia Guidance",
              content = "You must deselect 'none' to choose a premium. If you reselect 'none', it will deselect all other options. You can select two options, but only if one of them is Academia.",
  ...
  observeEvent(input$calculate, {
    ...
    income_tax <- function(gross) {
      ...
      if (gross > TAPER_START && gross <= TAPER_END) pa <- max(0, PERSONAL_ALLOWANCE - ((gross - TAPER_START) / 2))
      if (gross > TAPER_END) pa <- 0
      ti <- max(0, gross - pa)
      if (ti <= BASIC_BAND) return(ti * BASIC_RATE)
      if (ti <= TAPER_END) return(BASIC_BAND * BASIC_RATE + (ti - BASIC_BAND) * HIGHER_RATE)
      return(BASIC_BAND * BASIC_RATE + (TAPER_END - BASIC_BAND) * HIGHER_RATE + (ti - TAPER_END) * ADDITIONAL_RATE)
    }

    ni_calc <- function(gross) {
      ti <- gross - NI_THRESHOLD
      if (ti <= 0) return(0)
      if (ti <= NI_BASIC_LIMIT) return(ti * NI_RATE1)
      return(NI_BASIC_LIMIT * NI_RATE1 + (ti - NI_BASIC_LIMIT) * NI_RATE2)
    }
