#1.1.4 - made changes in preparation for adding other country options (moved some stuff round and added a welsh pay data frame(this could be a non-english-pay data frame since all 3 other nations use some variant of the 2002 contract)) + corrected some spelling errors + added comments to explain what further changes need to be made in case I forget what I was doing or someone else wants to finish this project
...
Banding <- data.frame(
	pay_band_mult = c("no band","1C","1B","1A","2B","2A","3","FC","FB","FA"),
	pay_band_mult =c (1,1.2,1.4,1.5,1.5,1.8,2,1.2,1.4,1.5)
)
...
# UI	
ui <- bslib::page_fluid(
  titlePanel("Resident Doctor Pay Uplift Calculator - England/Wales 25/26 v1.1.4"),
  sidebarLayout(
    sidebarPanel(
      width = 4,
# Country selector
radioButtons(
  "nation", "Where do you work? (as of v1.1.4 only England works)",
...
#Pay options for outside England	  
	  conditionalPanel(
		condition = "input.nation != 'England'",
      h6(strong("Select Grade")),
					        radioButtons(
							"grade_notEng", "What's your grade?",
					      ...
					      h6(strong("Select Incremental Pay")),
					        radioButtons(
							"pay_notEng", "Within your grade, what incrmental point are you at?",
	  ...
	  tagList(
		h6(strong("LTFT Options")),
		p("If you’re working less than full time (e.g. 80%), tick the box with your LTFT percentage",
			class = "text-muted", style = "margin-top: -8px; margin-bottom: 8px; font-size: 0.9em;")
		),
      checkboxInput("is_ltft", "Are you less than full time?", value = FALSE),
      conditionalPanel(
        condition = "input.is_ltft == true",
        radioButtons(
			"LTFT_notEng", "What percentage are you?",
			choices = c("90%", "80%", "70%", "60%", "50%", "not LTFT"),
			selected = "80%",
			inline = TRUE
		),
		numericInput("ltft_hoursA", "LTFT Total Hours (use 40 if Full time)", value = 40, min = 0, max = 56),
      ...
      h6(strong("Banding Options")),
		checkboxInput("know_band", "Do you know your pay band? If not we'll assume band 1C", value = TRUE),
			conditionalPanel(
				condition = "input.know_band == true",
					        radioButtons(
							"Pay_Band", "What's your band?",
							choices = c("no band", "1C", "1B", "1A", "2B", "2A", "3"),
							selected = "80",
							inline = TRUE
					),				
			)
...
# try to condense the 2 observes below into one thing)
	observe({
  ...
  observeEvent(input$calculate, {
   ...
   #LTFT stuff is calculated first because it is necessary for all nation options (cos of pension factor)
    #ltft
...
#Should only bother with the stuff down to "#change to welsh pay" if input.nation = England (but still handle nulls somehow)	
	...
	#if LTFT multiply the gross figures above by the LTFT percent and round up to the nearest integer
