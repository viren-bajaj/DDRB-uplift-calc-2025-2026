...
# UI	
ui <- bslib::page_fluid(
  titlePanel("Resident Doctor Pay Uplift Calculator - England 25/26 v1.1.1"),
  sidebarLayout(
    sidebarPanel(
      width = 4,
...
server <- function(input, output, session) {
  ...
  observeEvent(input$calculate, {
	...
	new_enhanced_pay <- input$enhanced_hours * base_hourly_new * enhanced_multiplier * 52
	new_non_enhanced_pay <- basic_hours * base_hourly_new * non_enhanced_multiplier * 52
	new_adj_weekend_pay <- weekend_supplement(ft_weekend_freq, uplifted_grade) * weekend_freq_percent
	new_adj_on_call <- weekend_freq_percent * (uplifted_grade * 0.08)
	}
    ...
    output$salary_output <- renderPrint({
	 ...
	 if (!input$know_gross_income) {  # Show detailed OOH breakdown only if gross income wasn't entered
			cat("   OOH Pay: £", formatC(OOH_pay, format = "f", digits = 2), " is made of the following. If there has been an error with the inputs (or you are LTFT) these values will be wrong:\n")
		if (enhanced_pay != 0) cat("   OOH Pay - Enhanced Hours: £", formatC(enhanced_pay, format = "f", digits = 2), "\n")
		if (adj_weekend_pay != 0) cat("   OOH Pay - Weekend Allowance: £", formatC(adj_weekend_pay, format = "f", digits = 2), "\n")
		if (adj_on_call != 0) cat("   OOH Pay - On-call Availability: £", formatC(adj_on_call, format = "f", digits = 2), "\n")
	} else {
	 ...
	 if (!input$know_gross_income) {  # Show detailed OOH breakdown only if gross income wasn't entered
			cat("   OOH Pay: £", formatC(new_OOH, format = "f", digits = 2), " is made of the following. If there has been an error with the inputs (or you are LTFT) these values may be wrong:\n")
		if (new_enhanced_pay != 0) cat("   OOH Pay - Enhanced Hours: £", formatC(new_enhanced_pay, format = "f", digits = 2), "\n")
		if (new_adj_weekend_pay != 0) cat("   OOH Pay - Weekend Allowance: £", formatC(new_adj_weekend_pay, format = "f", digits = 2), "\n")
		if (new_adj_on_call != 0) cat("   OOH Pay - On-call Availability: £", formatC(new_adj_on_call, format = "f", digits = 2), "\n")
	} else {
		if (new_OOH != 0) cat("   OOH Pay: £", formatC(new_OOH, format = "f", digits = 2), "\n")
	}
