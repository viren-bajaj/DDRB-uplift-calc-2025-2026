...
ui <- bslib::page_fluid(
  ...
  titlePanel("Resident Doctor Pay Uplift Calculator - England/Wales 25/26 v1.1.8.4"),
  sidebarLayout(
    sidebarPanel(
      width = 4,
# Country selector
...
server <- function(input, output, session) {
  ...
  vals <- reactiveValues(salary_html = NULL) # prevent output changes if someone changes input before pressing calc
	...
	observeEvent(input$nation, {
		# Always uncheck the common LTFT toggle
		updateCheckboxInput(session, "is_ltft", value = FALSE)

		# Reset England-specific LTFT inputs
		updateNumericInput(session, "ltft_percentageA", value = 100)

		# Reset non-England LTFT inputs
		updateRadioButtons(session, "LTFT_notEng", selected = "not LTFT")
		updateNumericInput(session, "ltft_hoursA", value = 40)
	})
...
#GP band overrides LTFT banding, i think (but unclear how it interacts with the F5-F9 stuff), if GP box is ticked, this won't even appear
  ...
  observeEvent(input$calculate, {
   ...
   #Common inputs/ things that break the app if not zero'd
	...
	if (input$nation == "England") {
		ltft_hours <- if (input$is_ltft && !is.null(input$non_enhanced_hours) && input$non_enhanced_hours > 0) input$non_enhanced_hours else 40
	} else	
	if (input$nation != "England") {
...
band_percent = (band_mult-1) * 100
  ...
  # Old Pay Section
  out <- c(out, "<h4>Old Pay</h4>")
  out <- c(out, sprintf("<p><strong>Gross Pay:</strong> %s<br><em>Pensionable:</em> %s</p>", fmt(gross_pay), fmt(pensionable)))

  # Breakdown (England detailed)
  if (input$nation == "England" && !input$know_gross_income) {
    out <- c(out, "<ul>")
    ...
    } else if (input$nation == "England" && input$know_gross_income) {
    out <- c(out, "<ul>")
    out <- c(out, sprintf("<li>Base Pay: %s</li>", fmt(adj_grade)))
    if (OOH_pay > 0)out <- c(out, sprintf("<li>OOH Pay: %s</li>", fmt(OOH_pay)))
    out <- c(out, "</ul>")
	# Breakdown (not England detailed)
  } else if (input$nation != "England" && band_mult > 1) {
    out <- c(out, "<ul>")
    if (unband_old > 0) out <- c(out, sprintf("<li>Base Pay: %s</li>", fmt(unband_old)))
    out <- c(out, sprintf("<li>Banding Uplift (%s%%): %s</li>", band_percent, fmt(band_enhance_old)))
    out <- c(out, "</ul>")
  }
  ...
  if (pension_ded > 0) out <- c(out, sprintf("<li>Pension: %s</li>", fmt(pension_ded)))
  if (student_loan_old > 0) out <- c(out, sprintf("<li>Student Loan: %s</li>", fmt(student_loan_old)))
  out <- c(out, sprintf("<li>Income Tax: %s</li>", fmt(income_tax_old)))
  out <- c(out, sprintf("<li>National Insurance: %s</li>", fmt(ni_old)))
  out <- c(out, "</ul>")

  out <- c(out, sprintf("<p><strong>Net Pay:</strong> %s</p>", fmt(net_old)))
  if (pension_accrued_old > 0) out <- c(out, sprintf("<p><em>Pension Accrued:</em> %s</p>", fmt(pension_accrued_old)))

  # New Pay
  out <- c(out, "<hr><h4>New Pay (Uplifted)</h4>")
  out <- c(out, sprintf("<p><strong>Gross Pay:</strong> %s<br><em>Pensionable:</em> %s</p>", fmt(new_gross), fmt(new_pensionable)))

  if (input$nation == "England" && !input$know_gross_income) {
    out <- c(out, "<ul>")
    out <- c(out, sprintf("<li>Regular Pay (%s hrs): %s</li>", total_time, fmt(new_non_enhanced_pay)))
    if (new_enhanced_pay > 0) out <- c(out, sprintf("<li>Enhanced Pay (%s hrs @ 37%%): %s</li>", enhanced_time, fmt(new_enhanced_pay)))
    if (new_adj_weekend_pay > 0) out <- c(out, sprintf("<li>Weekend Allowance: %s</li>", fmt(new_adj_weekend_pay)))
    if (new_adj_on_call > 0) out <- c(out, sprintf("<li>On-call Availability: %s</li>", fmt(new_adj_on_call)))
    out <- c(out, "</ul>")
    
	} else if (input$nation == "England" && input$know_gross_income) {
    out <- c(out, "<ul>")
    out <- c(out, sprintf("<li>Base Pay: %s</li>", fmt(new_adj_grade)))
    if (new_OOH > 0)out <- c(out, sprintf("<li>OOH Pay: %s</li>", fmt(new_OOH)))
    out <- c(out, "</ul>")
	
	# Breakdown (not England detailed)
  } else if (input$nation != "England" && band_mult > 1) {
    out <- c(out, "<ul>")
    if (unband_new > 0) out <- c(out, sprintf("<li>Base Pay: %s</li>", fmt(unband_new)))
    out <- c(out, sprintf("<li>Banding Uplift (%s%%): %s</li>", band_percent, fmt(band_enhance_new)))
    out <- c(out, "</ul>")
  }
 
 # Add universal items for England 
	if (input$nation == "England") {
		if (new_premia > 0) out <- c(out, sprintf("<p>Pay Premia: %s</p>", fmt(new_premia)))
		if (adj_weight > 0) out <- c(out, sprintf("<p>London Weighting: %s</p>", fmt(adj_weight)))
		if (ltft_premia > 0) out <- c(out, sprintf("<p>LTFT Allowance: %s</p>", fmt(ltft_premia)))
	}

  out <- c(out, "<p><strong>Deductions:</strong></p><ul>")
  if (new_pension_ded > 0) out <- c(out, sprintf("<li>Pension: %s</li>", fmt(new_pension_ded)))
  if (student_loan_new > 0) out <- c(out, sprintf("<li>Student Loan: %s</li>", fmt(student_loan_new)))
  out <- c(out, sprintf("<li>Income Tax: %s</li>", fmt(income_tax_new)))
  out <- c(out, sprintf("<li>National Insurance: %s</li>", fmt(ni_new)))
  out <- c(out, "</ul>")

  out <- c(out, sprintf("<p><strong>Net Pay:</strong> %s</p>", fmt(net_new)))
  if (pension_accrued_new > 0) out <- c(out, sprintf("<p><em>Pension Accrued:</em> %s</p>", fmt(pension_accrued_new)))

  # Uplift summary
  out <- c(out, "<hr><h4>Overall Uplift</h4><ul>")
  out <- c(out, sprintf("<li>Uplift to Basic Pay: %.2f%%</li>", basic_uplift))
  out <- c(out, sprintf("<li>Uplift to Gross Pay: %.2f%%</li>", gross_uplift))
  out <- c(out, sprintf("<li>Uplift to Net Pay: %.2f%%</li>", net_uplift))
  out <- c(out, sprintf("<li><strong>Net Pay Difference:</strong> %s</li>", fmt(net_new - net_old)))
  out <- c(out, "</ul>")

	#stroing values for the barplots
	vals$net_old <- net_old
	vals$net_new <- net_new
	vals$pensionable <- pensionable
	vals$new_pensionable <- new_pensionable
	vals$gross_pay <- gross_pay
	vals$new_gross <- new_gross
	vals$pension_ded <- pension_ded
	vals$new_pension_ded <- new_pension_ded
	vals$student_loan_old <- student_loan_old
	vals$student_loan_new <- student_loan_new
	vals$income_tax_old <- income_tax_old
	vals$income_tax_new <- income_tax_new
	vals$ni_old <- ni_old
	vals$ni_new <- ni_new
 ...
 output$salary_output <- renderUI({
	...
	req(vals$pensionable, vals$gross_pay, vals$new_pensionable, vals$new_gross)
	
		if (vals$pensionable > (vals$gross_pay + 0.00001) || 
			vals$new_pensionable > (vals$new_gross + 0.00001)) {
			HTML(
			'<div class="alert alert-warning" role="alert">
				⚠️ <strong>Note:</strong> If your pensionable pay is higher than your gross pay.
    ...
    net_values <- c(vals$net_old, vals$net_new)
		deduction_values <- c(
			vals$pension_ded + vals$student_loan_old + vals$income_tax_old + vals$ni_old,
			vals$new_pension_ded + vals$student_loan_new + vals$income_tax_new + vals$ni_new
		)
