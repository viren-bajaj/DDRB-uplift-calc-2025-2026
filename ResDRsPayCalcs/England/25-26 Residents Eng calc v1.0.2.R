  ...
  observeEvent(input$calculate, {
    ...
    income_tax <- function(gross, is_scotland = FALSE) {
      ...
      if (!is_scotland) {
        ...
        bands <- c(SCOT_STARTER_THRESH, SCOT_BASIC_THRESH, SCOT_INTERMEDIATE_THRESH,
                   SCOT_HIGHER_THRESH, SCOT_ADVCANCED_THRESH)
        rates <- c(SCOT_STARTER_RATE, SCOT_BASIC_RATE, SCOT_INTERMEDIATE_RATE,
                   SCOT_HIGHER_RATE, SCOT_ADVANCED_RATE, SCOT_TOP_RATE)
        
        lower <- SCOT_INCOME_THRESH
        
        for (i in seq_along(rates)) {
          upper <- if (i <= length(bands)) bands[i] else Inf
          band_width <- min(remaining, upper - lower)
          
          if (band_width > 0) {
            tax_due <- tax_due + band_width * rates[i]
            remaining <- remaining - band_width
            lower <- upper
          } else {
            break
          }
